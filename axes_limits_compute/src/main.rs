use std::{
    error::Error,
    fs::File,
    io::{Read, Write, read_to_string, stdin},
    path::{Path, PathBuf},
};
...
fn find_plugin_dir() -> Result<PathBuf, Box<dyn Error>> {
    ...
    Ok(current.join("extended_color_selector"))
}

fn get_axes_limit_segments(plugin: &Path) -> Result<u32, Box<dyn Error>> {
    let config = read_to_string(File::open(plugin.join("config.py"))?)?;
    let line = config
        .lines()
        .find(|line| line.starts_with("AXES_LIMITS_SEGMENTS"))
        .ok_or("Unable to parse config file")?;
    let start = line
        .chars()
        .collect::<Vec<_>>()
        .into_iter()
        .rposition(|c| c == '=')
        .ok_or("Unable to parse config file")?;
    Ok(u32::from_str_radix(&line[start + 1..].trim(), 10)?)
}
...
fn main() {
    ...
    let segments = get_axes_limit_segments(&plugin_dir).unwrap();
    println!("Found segments in config: {}", segments);
    ...
    println!("Done! Now you can close this window!");
